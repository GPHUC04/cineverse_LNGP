...
//    tableModel.setRowCount(0); // Xóa tất cả các hàng trong bảng
//
//    // Lấy danh sách lịch chiếu từ CSDL
//    for (model.Schedule_model s : scheduleDAO.getSchedules()) {
//        // Chuyển các kiểu dữ liệu LocalDate và LocalTime sang chuỗi
//        String sdateString = s.getSdate().toString(); // Chuyển LocalDate sang String
//        String stimeString = s.getStime().toString(); // Chuyển LocalTime sang String
//        
//        // Đảm bảo thời gian có định dạng HH:mm:ss nếu là kiểu Time
//        // Nếu muốn hiển thị theo định dạng cụ thể, có thể sử dụng DateTimeFormatter
//        if (stimeString.length() == 8) {  // Đảm bảo chỉ hiển thị thời gian đúng định dạng HH:mm:ss
//            stimeString = stimeString.substring(0, 5); // Chỉ lấy HH:mm
//        }
//
//        // Thêm dữ liệu vào bảng
//        String[] rowData = {
//            s.getScid(),                   // Mã lịch chiếu
//            s.getMid(),                    // Mã phim
//            s.getRid(),                    // Mã phòng chiếu
//            sdateString,                   // Ngày chiếu
//            stimeString,                   // Thời gian chiếu (đảm bảo đúng định dạng)
//            String.valueOf(s.getPrice())   // Giá vé
//        };
//        
...
public void setData() {
    ...
    for (model.Schedule_model s : scheduleDAO.getSchedules()) {
        // Chuyển các kiểu dữ liệu LocalDate và LocalTime sang chuỗi
        String sdateString = s.getSdate().toString(); // Chuyển LocalDate sang String
        LocalTime stime = s.getStime(); // Lấy LocalTime từ Schedule_model

        // Trừ 2 giờ chỉ khi lấy dữ liệu từ cơ sở dữ liệu
        stime = stime.minusHours(1);  // Trừ 2 giờ từ thời gian

        // Định dạng lại thời gian theo HH:mm:ss
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("HH:mm:ss");
        String stimeString = stime.format(formatter);  // Chuyển LocalTime sang chuỗi định dạng HH:mm:ss
        ...
        String[] rowData = {
            ...
            s.getRid(),                    // Mã phòng chiếu
            sdateString,                   // Ngày chiếu
            stimeString,                   // Thời gian chiếu đã được điều chỉnh
            String.valueOf(s.getPrice())   // Giá vé
        };
    ...
    public void showInsertdialog() {
        // trước khi insert thì viết hàm xoá dữ liệu cũ , dỗ dữ liệu từ CSDL vào combobox
        tf_schedule1.setText(""); // Xóa nội dung trường schedule ID
        cb_movie1.removeAllItems(); // Xóa tất cả các mục trong ComboBox movie
        cb_room1.removeAllItems(); // Xóa tất cả các mục trong ComboBox room
        Jdate1.setDate(null); // Xóa ngày đã chọn trong JDateChooser
        tf_time1.setText(""); // Xóa nội dung trường thời gian
        tf_price1.setText(""); // Xóa nội dung trường giá
        

         ArrayList<String> list = scheduleDAO.loaddatacomboboxMovie();
         for (String item : list) {
             cb_movie1.addItem(item);  // Thêm mỗi bộ phim vào ComboBox
         }

         // Lấy dữ liệu cho ComboBox room và thêm vào cb_room2
         ArrayList<String> list1 = scheduleDAO.loaddatacomboboxRoom();
         for (String item : list1) {
             cb_room1.addItem(item);  // Thêm mỗi phòng vào ComboBox
         }
...
//        Jdate2.setDate(date);
//        tf_time2.setText(schedule.getStime().toString());
...
//          }
    ...
    // Lấy dữ liệu cho ComboBox movie và thêm vào cb_movie2
    ArrayList<String> list = scheduleDAO.loaddatacomboboxMovie();
    for (String item : list) {
        cb_movie2.addItem(item);  // Thêm mỗi bộ phim vào ComboBox
    }

    // Lấy dữ liệu cho ComboBox room và thêm vào cb_room2
    ArrayList<String> list1 = scheduleDAO.loaddatacomboboxRoom();
    for (String item : list1) {
        cb_room2.addItem(item);  // Thêm mỗi phòng vào ComboBox
    }

    
    
    
    // Chuyển đổi LocalDate sang Date cho JDateChooser
    LocalDate localDate = schedule.getSdate();
    Date date = (localDate != null) ? Date.from(localDate.atStartOfDay(ZoneId.systemDefault()).toInstant()) : null;
    
    // Điền các trường với dữ liệu của lịch chiếu đã chọn
    tf_schedule2.setText(schedule.getScid());
    cb_movie2.setSelectedItem(schedule.getMid());
    cb_room2.setSelectedItem(schedule.getRid());
    Jdate2.setDate(date);
...
//        int price = Integer.parseInt(tf_price1.getText());
//        
//         Schedule_model s = new Schedule_model();
//        s.setScid(tf_schedule1.getText());
//        s.setMid(cb_movie1.getSelectedItem().toString());
//        s.setRid(cb_room1.getSelectedItem().toString());
//        s.setSdate(sdate);
//        s.setStime(stime);
//        s.setPrice(price);
//        
...
//        
//        String scheduleID = model.getValueAt(selectedRow, 0)+ "";
//        String movieID = model.getValueAt(selectedRow, 1)+"";
//        String roomID = model.getValueAt(selectedRow, 2)+ "";
...
//        int price = Integer.valueOf(model.getValueAt(selectedRow, 5) +"");
//        
//        
//        
//        
...
//        }
    ...
    if (selectedRow == -1) { // Kiểm tra nếu không có dòng nào được chọn
        return null; // Trả về null nếu không có dòng nào được chọn
    }
  ...
  public void insertSchedule() {
    ...
    // Chuyển đổi dữ liệu ngày và thời gian từ giao diện
    LocalDate sdate = Jdate1.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate(); // Lấy LocalDate từ JDateChooser
    LocalTime stime = LocalTime.parse(tf_time1.getText()); // Lấy LocalTime từ TextField
     // Thêm 1 giờ vào thời gian nhập
    stime = stime.plusHours(1);  // Cộng thêm 1 giờ
    float price = Float.valueOf(tf_price1.getText()); // Lấy giá từ TextField

    // Tạo đối tượng Schedule_model với dữ liệu đã nhập
    Schedule_model s = new Schedule_model();
    s.setScid(scid);
    s.setMid(mid);
    s.setRid(rid);
    ...
    // Gọi phương thức để thêm lịch chiếu vào cơ sở dữ liệu
    scheduleDAO.addSchedule(s);

    // Cập nhật lại dữ liệu hiển thị sau khi thêm lịch chiếu
     //scheduleDAO.setScheduleData(); 
     setData();
    // Hiển thị thông báo thêm lịch chiếu thành công
    JOptionPane.showMessageDialog(jDialog_insert, "Schedule has been added!", "Adding Successfully", JOptionPane.PLAIN_MESSAGE);
    
    // Đóng hộp thoại thêm lịch
    jDialog_insert.setVisible(false);
}


public void editSchedule() {
    // Lấy dữ liệu từ các trường trong giao diện người dùng
    String scid = tf_schedule2.getText();  // scid là kiểu String nhưng sẽ chuyển thành INT trong cơ sở dữ liệu
    ...
    LocalTime stime = LocalTime.parse(tf_time2.getText());  // Thời gian nhập vào từ người dùng
    ...
    s.setRid(rid);
    s.setSdate(sdate);
    s.setStime(stime);  // Lưu trữ giờ đã cộng thêm 1 giờ
    s.setPrice(price);

    // Kiểm tra xem lịch chiếu có tồn tại không trước khi chỉnh sửa
    if (!scheduleDAO.isExistschedule(s.getScid())) {
    ...
    // Thực hiện cập nhật lịch chiếu trong cơ sở dữ liệu
    scheduleDAO.editSchedule(s);
    setData(); // Cập nhật dữ liệu hiển thị sau khi chỉnh sửa
